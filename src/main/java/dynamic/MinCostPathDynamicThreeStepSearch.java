...
public class MinCostPathDynamicThreeStepSearch {
    ...
    public static int search(int[][] cellCosts, int row, int column) {
        ...
        for (int rowIndex = 1; rowIndex <= row; rowIndex++) {
            for (int columnIndex = 1; columnIndex <= column; columnIndex++) {
                ...
                int diagonallyStepCost = minCellCosts[rowIndex - 1][columnIndex - 1];
                minCellCosts[rowIndex][columnIndex] = Math.min(
                        Math.min(downStepCost, rightStepCost), diagonallyStepCost) + cellCosts[rowIndex][columnIndex];
            }
        }
